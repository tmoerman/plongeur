...
import org.tmoerman.plongeur.tda.Skeleton.combineCoordinates

/**
  * @author Thomas Moerman
  */
class CombineCoordinatesTest extends FlatSpec with Matchers {

  behavior of "combining coordinates"

  it should "be correct with 1x1 interval coordinates" in {
    combineCoordinates(Seq(Seq(1), Seq("a"))) shouldBe Set(

      Vector(1, "a"))
  }

  it should "be correct with 1x2 interval coordinates" in {
    combineCoordinates(Seq(Seq(1), Seq("a", "b"))) shouldBe Set(

      Vector(1, "a"), Vector(1, "b"))
  }

  it should "be correct with 2x1 interval coordinates" in {
    combineCoordinates(Seq(Seq(1, 2), Seq("a"))) shouldBe Set(

      Vector(1, "a"), Vector(2, "a"))
  }

  it should "be correct with 2x2 interval coordinates" in {
    combineCoordinates(Seq(Seq(1, 2), Seq("a", "b"))) shouldBe Set(
  ...
  it should "be correct with 3x3 interval coordinates" in {
    combineCoordinates(Seq(Seq(1, 2, 3), Seq("a", "b", "c"))) shouldBe Set(
  ...
  it should "be correct with 2x2x2 interval coordinates" in {
    combineCoordinates(Seq(Seq(1, 2), Seq("a", "b"), Seq('X, 'Y))) shouldBe Set(
